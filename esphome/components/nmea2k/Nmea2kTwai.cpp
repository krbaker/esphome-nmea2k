...
Nmea2kTwai::Nmea2kTwai(gpio_num_t _TxPin,  gpio_num_t _RxPin, unsigned long recP, unsigned long logP):
     tNMEA2000(),TxPin(_TxPin),RxPin(_RxPin)
{
    if (RxPin < 0 || TxPin < 0){
        disabled=true;    
    }
...
bool Nmea2kTwai::CANSendFrame(unsigned long id, unsigned char len, const unsigned char *buf, bool wait_sent)
{
    ...
    if (rt != ESP_OK){
        ...
        ESP_LOGD("custom", "twai transmit for %ld failed: %x",LOGID(id),(int)rt);
        return false;
    }
    txTimeouts=0;
...
{
    if (disabled){
        ESP_LOGD("custom", "CAN disabled");
        return true;
    }
    esp_err_t rt=twai_start();
    if (rt != ESP_OK){
        ESP_LOGD("custom", "CANOpen failed: %x",(int)rt);
        return false;
    }
    else{
        ESP_LOGD("custom", "CANOpen ok");
    }
    return true;
}
bool Nmea2kTwai::CANGetFrame(unsigned long &id, unsigned char &len, unsigned char *buf)
{
    ...
    if (len > 8){
        ESP_LOGD("custom", "twai: received invalid message %lu, len %d",LOGID(id),len);
        len=8;
    }
    //ESP_LOGD("custom", "twai rcv id=%u,len=%d, ext=%d",LOGID(message.identifier),message.data_length_code,message.extd);
    if (! message.rtr){
...
void Nmea2kTwai::initDriver(){
    ...
    if (rt == ESP_OK) {
        ESP_LOGD("custom", "twai driver initialzed, rx=%d,tx=%d",(int)RxPin,(int)TxPin);
    }
    else{
        ESP_LOGD("custom", "twai driver init failed: %x",(int)rt);
    }
}
...
{
    if (disabled){
        ESP_LOGD("custom", "twai init - disabled");
    }
    else{
        initDriver();
    }
...
bool Nmea2kTwai::checkRecovery(){
    ...
    if (canState.state != Nmea2kTwai::ST_RUNNING)
    {
        if (canState.state == Nmea2kTwai::ST_BUS_OFF)
        {
            ...
            ESP_LOGD("custom",  "twai BUS_OFF: start can recovery - result %d", (int)rt);
        }
        if (canState.state == Nmea2kTwai::ST_STOPPED)
        {
            bool rt = CANOpen();
            ESP_LOGD("custom",  "twai STOPPED: restart can driver - result %d", (int)rt);
        }
    }
    return strt;
}
...
Nmea2kTwai::Status Nmea2kTwai::logStatus(){
    Status canState=getStatus();
//    ESP_LOGD("custom", "twai state %s, rxerr %d, txerr %d, txfail %d, txtimeout %d, rxmiss %d, rxoverrun %d",
...
bool Nmea2kTwai::startRecovery(){
    if (disabled) return false;
    lastRecoveryStart=esphome::millis();
    esp_err_t rt=twai_driver_uninstall();
    if (rt != ESP_OK){
        ESP_LOGD("custom", "twai: deinit for recovery failed with %x",(int)rt);
    }
    initDriver();
    bool frt=CANOpen();
    return frt;
